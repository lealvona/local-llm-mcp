...
CLIENT_ASSIST = """ACTIVE MODE: ASSIST. Delegate OFTEN: every step that needs no conversational context and whose raw output would exceed the digest you need. Rule of thumb: a direct call returning more than ~40 lines / 2 KB goes here instead. local_llm_run replaces your shell tool for logs, journals, test/build/lint runs, listings, git log/diff/status, grep/find results, systemctl/docker output, long dumps. local_llm_delegate for file reads/summaries, research over material, calculations, transformations, parsing, boilerplate. Do NOT delegate what prints little or nothing (mkdir, cp, rm, one-line checks), files under ~40 lines, or edits. For exact text use verbatim=true or local_llm_artifact, not your own reader. Exact counts/sorts/sums: compute in the command (wc, sort -n, awk, jq). Secrets and account/id numbers always come back masked; the first time names/emails appear the server asks the USER (a dialog) whether to show them — honour that answer, and call local_llm_disclosure when the user says so in chat."""
